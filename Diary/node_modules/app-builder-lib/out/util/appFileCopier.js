...
async function computeNodeModuleFileSets(platformPackager, mainMatcher) {
    ...
    let deps = [];
    const searchDirectories = Array.from(new Set([appDir, projectDir, await packager.getWorkspaceRoot()])).filter((it) => !(0, builder_util_1.isEmptyOrSpaces)(it));
    for (const dir of searchDirectories) {
        if (cancellationToken.cancelled) {
            throw new Error("user cancelled");
        }
        const dirDeps = await (0, node_module_collector_1.getNodeModules)(await packager.getPackageManager(), { rootDir: dir, tempDirManager, cancellationToken, packageName: packager.metadata.name });
        if (dirDeps.length > 0) {
            builder_util_1.log.debug({ dir, nodeModules: dirDeps }, "collected node modules");
            deps = dirDeps;
            break;
        }
